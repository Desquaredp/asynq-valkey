...
var SortRedisZSetEntryOpt = cmp.Transformer("SortZSetEntries", func(in []valkey.Z) []valkey.Z {
	out := append([]valkey.Z(nil), in...) // Copy input to avoid mutating it
	sort.Slice(out, func(i, j int) bool {
...
func FlushDB(tb testing.TB, r valkey.UniversalClient) {
	...
	case *valkey.ClusterClient:
		err := r.ForEachMaster(context.Background(), func(ctx context.Context, c *valkey.Client) error {
...
func seedRedisList(tb testing.TB, c valkey.UniversalClient, key string,
	...
	for _, msg := range msgs {
		...
		if len(msg.UniqueKey) > 0 {
			...
			if err != nil {
				tb.Fatalf("Failed to set unique lock in valkey: %v", err)
			}
		}
	}
}

func seedRedisZSet(tb testing.TB, c valkey.UniversalClient, key string,
	items []base.Z, state base.TaskState) {
	tb.Helper()
	for _, item := range items {
		msg := item.Message
		encoded := MustMarshal(tb, msg)
		z := valkey.Z{Member: msg.ID, Score: float64(item.Score)}
...
// Retrieves all messages stored under `keyFn(qname)` key in valkey list.
func getMessagesFromList(tb testing.TB, r valkey.UniversalClient, qname string,
...
// Retrieves all messages stored under `keyFn(qname)` key in valkey zset (sorted-set).
func getMessagesFromZSet(tb testing.TB, r valkey.UniversalClient, qname string,
	keyFn func(qname string) string, state base.TaskState) []*base.TaskMessage {
	tb.Helper()
...
// Retrieves all messages along with their scores stored under `keyFn(qname)` key in valkey zset (sorted-set).
func getMessagesFromZSetWithScores(tb testing.TB, r valkey.UniversalClient,
...
func SeedTasks(tb testing.TB, r valkey.UniversalClient, taskData []*TaskSeedData) {
	for _, data := range taskData {
		...
		if err := r.HSet(ctx, key, v).Err(); err != nil {
			tb.Fatalf("Failed to write task data in valkey: %v", err)
		}
		if len(msg.UniqueKey) > 0 {
