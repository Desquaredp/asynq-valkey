...
func getRedisConnOpt(tb testing.TB) ValkeyConnOpt {
	tb.Helper()
	if useRedisCluster {
		addrs := strings.Split(redisClusterAddrs, ",")
		if len(addrs) == 0 {
		...
		return ValkeyClusterClientOpt{
			Addrs: addrs,
		}
	}
	return ValkeyClientOpt{
		Addr: redisAddr,
		DB:   redisDB,
	}
}
...
func TestParseRedisURI(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ValkeyClientOpt{Addr: "localhost:6379"},
		},
		{
			"rediss://localhost:6379",
			ValkeyClientOpt{Addr: "localhost:6379", TLSConfig: &tls.Config{ServerName: "localhost"}},
		},
		{
			"redis://localhost:6379/3",
			ValkeyClientOpt{Addr: "localhost:6379", DB: 3},
		},
		{
			"redis://:mypassword@localhost:6379",
			ValkeyClientOpt{Addr: "localhost:6379", Password: "mypassword"},
		},
		{
			"redis://:mypassword@127.0.0.1:6379/11",
			ValkeyClientOpt{Addr: "127.0.0.1:6379", Password: "mypassword", DB: 11},
		},
		{
			"redis-socket:///var/run/redis/redis.sock",
			ValkeyClientOpt{Network: "unix", Addr: "/var/run/redis/redis.sock"},
		},
		{
			"redis-socket://:mypassword@/var/run/redis/redis.sock",
			ValkeyClientOpt{Network: "unix", Addr: "/var/run/redis/redis.sock", Password: "mypassword"},
		},
		{
			"redis-socket:///var/run/redis/redis.sock?db=7",
			ValkeyClientOpt{Network: "unix", Addr: "/var/run/redis/redis.sock", DB: 7},
		},
		{
			"redis-socket://:mypassword@/var/run/redis/redis.sock?db=12",
			ValkeyClientOpt{Network: "unix", Addr: "/var/run/redis/redis.sock", Password: "mypassword", DB: 12},
		},
		{
	...
	for _, tc := range tests {
		got, err := ParseValkeyURI(tc.uri)
		if err != nil {
			t.Errorf("ParseValkeyURI(%q) returned an error: %v", tc.uri, err)
			continue
		}

		if diff := cmp.Diff(tc.want, got, cmpopts.IgnoreUnexported(tls.Config{})); diff != "" {
			t.Errorf("ParseValkeyURI(%q) = %+v, want %+v\n(-want,+got)\n%s", tc.uri, got, tc.want, diff)
		}
	}
}
...
func TestParseRedisURIErrors(t *testing.T) {
	...
	for _, tc := range tests {
		_, err := ParseValkeyURI(tc.uri)
		if err == nil {
			t.Errorf("%s: ParseValkeyURI(%q) succeeded for malformed input, want error",
				tc.desc, tc.uri)
		}
	}
}
